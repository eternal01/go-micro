...
func (l *DeleteRegionLogic) DeleteRegion(req *types.GatewayDeleteRegionRequest) (resp *types.GatewayDeleteRegionReply, err error) {
	regions, err := l.svcCtx.BasicRegionRpc.GetRegions(l.ctx, &basic.GetRegionsRequest{
		ParentId: req.Id,
	})
	if err != nil {
		return nil, err
	...
	_, err = l.svcCtx.BasicRegionRpc.DeleteRegion(l.ctx, &basic.DeleteRegionRequest{
		Id: req.Id,
	})
	if err != nil {
		return nil, err
